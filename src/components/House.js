...
export default function House({ name, color, points, members }) {
  ...
  return (
    <div className="flex flex-col items-center mx-2">
      ...
      <p className="sorts-mill-goudy-regular text-lg text-ink-secondary">{points} pts</p>

      {/* {members && members.length > 0 && (
        <ul className="sorts-mill-goudy-regular text-xs mt-2 max-h-24 text-center text-ink-muted">
          {members.map((m, i) => (
            <li key={i}>{m}</li>
          ))}
        </ul>
